...
class MyStorySettingsRepository {
  ...
  fun getPrivacyState(): Single<MyStoryPrivacyState> = Single.fromCallable {
    getStoryPrivacyState()
  }.subscribeOn(Schedulers.io())

  fun observeChooseInitialPrivacy(): Observable<ChooseInitialMyStoryMembershipState> = Single
  ...
  fun setPrivacyMode(privacyMode: DistributionListPrivacyMode): Completable = Completable.fromAction {
  ...
  fun getRepliesAndReactionsEnabled(): Single<Boolean> = Single.fromCallable {
    SignalDatabase.distributionLists.getStoryType(DistributionListId.MY_STORY).isStoryWithReplies
  }.subscribeOn(Schedulers.io())

  fun setRepliesAndReactionsEnabled(repliesAndReactionsEnabled: Boolean): Completable = Completable.fromAction {
    SignalDatabase.distributionLists.setAllowsReplies(DistributionListId.MY_STORY, repliesAndReactionsEnabled)
    Stories.onStorySettingsChanged(DistributionListId.MY_STORY)
  }.subscribeOn(Schedulers.io())

  fun getAllSignalConnectionsCount(): Single<Int> = Single.fromCallable {
