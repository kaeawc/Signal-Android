...
class StoryDirectReplyRepository(context: Context) {
  ...
  fun getStoryPost(storyId: Long): Single<MessageRecord> = Single.fromCallable {
    SignalDatabase.messages.getMessageRecord(storyId)
  }.subscribeOn(Schedulers.io())

  fun send(storyId: Long, groupDirectReplyRecipientId: RecipientId?, body: CharSequence, bodyRangeList: BodyRangeList?, isReaction: Boolean): Completable = Completable.create { emitter ->
