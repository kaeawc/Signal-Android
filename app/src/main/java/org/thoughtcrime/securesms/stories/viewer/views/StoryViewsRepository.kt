...
class StoryViewsRepository {
  ...
  fun getStoryRecipient(storyId: Long): Single<Recipient> = Single.fromCallable {
    SignalDatabase.messages.getMessageRecord(storyId).toRecipient
  }.subscribeOn(Schedulers.io())

  fun getViews(storyId: Long): Observable<List<StoryViewItemData>> = Observable.create<List<StoryViewItemData>> { emitter ->
  ...
  fun removeUserFromStory(user: Recipient, story: Recipient): Completable = Completable.fromAction {
