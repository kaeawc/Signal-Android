...
open class StoryViewerRepository {
  fun getFirstStory(recipientId: RecipientId, storyId: Long): Single<MmsMessageRecord> = if (storyId > 0) {
    Single.fromCallable {
  ...
  fun getStories(hiddenStories: Boolean, isOutgoingOnly: Boolean): Single<List<RecipientId>> = Single.create { emitter ->
  ...
  private fun List<RecipientId>.floatToTop(recipientId: RecipientId?): List<RecipientId> = if (recipientId != null && contains(recipientId)) {
