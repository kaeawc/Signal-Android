...
class MyStoriesRepository(context: Context) {
  ...
  fun getMyStories(): Observable<List<MyStoriesState.DistributionSet>> = RxDatabaseObserver
    .conversationList
    .toObservable()
    .map {
  ...
  private fun createDistributionSet(recipient: Recipient, messageRecords: List<MessageRecord>): MyStoriesState.DistributionSet = MyStoriesState.DistributionSet(
  ...
  class MyStoryBiasComparator : Comparator<Recipient> {
    override fun compare(o1: Recipient, o2: Recipient): Int = when {
      o1 == o2 -> 0
      o1.isMyStory -> -1
      o2.isMyStory -> 1
      else -> -1
