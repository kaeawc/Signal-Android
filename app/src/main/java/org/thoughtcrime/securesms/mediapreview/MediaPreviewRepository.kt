...
class MediaPreviewRepository {
   ...
   */
  fun getAttachments(context: Context, startingAttachmentId: AttachmentId, threadId: Long, sorting: Sorting, limit: Int = 500): Flowable<Result> = Single.fromCallable {
  ...
  fun localDelete(attachment: DatabaseAttachment): Completable = Completable.fromRunnable {
  ...
  fun remoteDelete(attachment: DatabaseAttachment): Completable = Completable.fromRunnable {
    MessageSender.sendRemoteDelete(attachment.mmsId)
  }.subscribeOn(Schedulers.io())

  fun getMessagePositionIntent(context: Context, messageId: Long): Single<Intent> = Single.fromCallable {
