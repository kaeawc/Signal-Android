...
sealed class MediaSelectionDestination {
  ...
  class MultipleRecipients(val recipientSearchKeys: List<ContactSearchKey.RecipientSearchKey>) : MediaSelectionDestination() {
    ...
    companion object {
      fun fromParcel(parcelables: List<ContactSearchKey.RecipientSearchKey>): MultipleRecipients = MultipleRecipients(parcelables)
    }

    override fun getRecipientSearchKey(): ContactSearchKey.RecipientSearchKey? = if (recipientSearchKeys.size == 1) {
  ...
  companion object {
    ...
    fun fromBundle(bundle: Bundle): MediaSelectionDestination = when {
