...
sealed class RegisterAccountResult(cause: Throwable?) : RegistrationResult(cause) {
  companion object {
    fun from(networkResult: NetworkResult<RegistrationRepository.AccountRegistrationResult>): RegisterAccountResult = when (networkResult) {
    ...
    private fun createRateLimitProcessor(exception: RateLimitException): RegisterAccountResult = if (exception.retryAfterMilliseconds.isPresent) {
