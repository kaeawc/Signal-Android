 ...
 */
class GroupArchiveExporter(private val cursor: Cursor) :
...
private fun GroupTable.ShowAsStoryState.toRemote(): Group.StorySendMode = when (this) {
...
private fun AccessControl.AccessRequired.toRemote(): Group.AccessControl.AccessRequired = when (this) {
...
private fun AccessControl.toRemote(): Group.AccessControl = Group.AccessControl(members = members.toRemote(), attributes = attributes.toRemote(), addFromInviteLink = addFromInviteLink.toRemote())

private fun Member.Role.toRemote(): Group.Member.Role = when (this) {
...
private fun DecryptedMember.toRemote(): Group.Member = Group.Member(userId = aciBytes, role = role.toRemote(), joinedAtVersion = joinedAtRevision)

private fun DecryptedPendingMember.toRemote(): Group.MemberPendingProfileKey = Group.MemberPendingProfileKey(
  member = Group.Member(
...
private fun DecryptedBannedMember.toRemote(): Group.MemberBanned = Group.MemberBanned(
  userId = this.serviceIdBytes,
  timestamp = this.timestamp
)

private fun DecryptedRequestingMember.toRemote(): Group.MemberPendingAdminApproval = Group.MemberPendingAdminApproval(
