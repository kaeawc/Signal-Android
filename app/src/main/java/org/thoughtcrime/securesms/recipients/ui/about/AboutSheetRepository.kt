...
class AboutSheetRepository {
  fun getGroupsInCommonCount(recipientId: RecipientId): Single<Int> = Single.fromCallable {
    SignalDatabase.groups.getPushGroupsContainingMember(recipientId).size
  }.subscribeOn(Schedulers.io())

  fun getVerified(recipientId: RecipientId): Single<Boolean> = Single.fromCallable {
